...
class Unfold:
    ...
    def __init__(
        ...
        perm_gen2sc: numpy.ndarray | None = None,
        verbose: bool = False,
    ):
        """
        Args:
            ...
            perm_sc2gen (numpy.ndarray, optional): Permutation from Phonopy supercell ordering
                to the ordering generated by ``make_supercell``.
            perm_gen2sc (numpy.ndarray, optional): Inverse of ``perm_sc2gen``.
    ...
    def prepare(self):
        ...
        if (self.perm_gen2sc is not None) and (self.perm_sc2gen is not None):
            assert isinstance(self.perm_gen2sc, numpy.ndarray)
            assert isinstance(self.perm_sc2gen, numpy.ndarray)
            assert self.perm_gen2sc.shape == self.perm_sc2gen.shape
            assert numpy.all(self.perm_gen2sc >= 0) and numpy.all(self.perm_sc2gen >= 0)
            if self.unfold_atoms_indices is None:
                assert len(self.perm_gen2sc) == len(self.sc_by_mat)
                assert len(self.perm_sc2gen) == len(self.uc)
            else:
                assert len(self.perm_gen2sc) == len(self.unfold_atoms_indices)
                assert len(self.perm_sc2gen) == len(self.unfold_atoms_indices)
            ...
            print("WARNING: it is strongly recommended to provide permutation indices perm_sc2gen and perm_gen2sc")
            ...
            self.perm_gen2sc = _match["atoms_indices_b2a"]
    ...
    def _calculate_weights_one_kpt(self, kpt_idx: int) -> numpy.ndarray:
        ...
        if self.perm_gen2sc is not None:
            uc2sc_modes = uc2sc_modes[self.perm_gen2sc]
            uc2sc_modes = uc2sc_modes.reshape(3 * len(self.perm_gen2sc), 3 * uc_natoms)
        else:
