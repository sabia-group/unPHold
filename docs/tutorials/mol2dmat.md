...
In this section, we unfold the phonons of a molecular array on top of a graphene monolayer, a heterostructure known as "mol2Dmat".
...
We use [MACE-MH-1](https://github.com/ACEsuit/mace-foundations) with D3 dispersion to relax the structure and construct the force constants (configurations available at [`tests/data/mol2dmat`](https://github.com/sabia-group/unPHold/blob/main/tests/data/mol2dmat)).
...
A phonon calculation on this unit cell (UC) gives 1248 entangled bands.
...
In what follows, "supercell" (SC) always refers to the ideal cell built by tiling a PC with a transformation matrix.
...
In the `Unfold` call, we pass the UC as the `supercell` argument, since from the PC's point of view, the calculated cell is its supercell.
...
We obtain the unfolded phonon spectral function by adding a Gaussian broadening with weights as amplitudes:
...
This recovers the graphene phonon dispersion as clear branches, with the molecular covalent modes above 80 THz removed. The spectral function below 10 THz is blurred, indicating strong coupling between the graphene and molecular phonon modes; a layer-breathing mode appears at 2.5 THz and is visualized below.
...
1. **Find and unwrap the molecules.**
    We group the atoms of the molecular layer into four clusters, one per molecule, using `ase.neighborlist`, then make each cluster contiguous in real space.
...
The VDOS below 10 THz differs markedly, indicating coupling between the molecular and graphene layers.
...
We can visualize the real-space motion of the coupled heterostructure.
...
<figure markdown>
  ...
  <figcaption>Mode 40 (2.490 THz, weight 0.249). This is the layer-breathing-mode-like phonon of this heterostructure: the collective motion dominates along the z-direction, and the two layers move against each other.</figcaption>
</figure>
