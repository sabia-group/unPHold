...
Different phonon mode branches have different physical character, which we quantify with metrics computed from the phonon eigenvectors.
...
No unfolding is involved here: the metrics are functions of the phonon eigenvectors of any cell, so we work directly with the 2-atom primitive cell.
For an application that combines the metrics with unfolding weights, see the [twisted bilayer graphene tutorial](twisted_bilayer.md).

---

## Preparing inputs

The metrics need phonon eigenvectors, so we must run the band structure with `with_eigenvectors=True`:
...
All metric functions accept eigenvectors of shape `(nqpoints, natoms*3, nbands)` and return an array of shape `(nqpoints, nbands)` with values between 0 and 1 for each mode.
...
In the figures below, each band appears as a line colored by the metric value of that mode, using a single-color colormap from light red (0) to dark red (1).
